...
| 📊 带货数据 | GMV、销量、视频/直播表现、粉丝画像（含性别占比） |
...
| 🧹 自动去重 | 已抓过的达人自动跳过，不重复入库 |
| 👥 多账号并发 | 多个 Cookie 同时抓取，自动错峰启动 |
| 🛡️ 会话轮换 | 触发风控自动换会话断点续抓（最多 3 次） |
...
**Q：中途断了怎么办？**  
A：再次启动并开始抓取，会自动从上次位置继续（断点续抓）；触发风控时会自动换会话继续，最多 3 次。
