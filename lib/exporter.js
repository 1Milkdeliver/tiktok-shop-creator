...
function normalizeValue(v) {
  ...
  if (v.key !== undefined && v.key !== null) {
    return String(v.key) + (v.value === undefined || v.value === null ? '' : ': ' + v.value);
  }
...
async function exportXlsx(filePath, rows, headers) {
  ...
  for (const r of rows) {
    const row = ws.addRow(headers.map(h => { const v = normalizeValue(r[h]); return v === '' ? '' : v; }));
    row.font = { name: 'Arial' }; // default font for data rows
  }
  // column widths
...
module.exports = { exportCsv, exportXlsx, mergeRows, ensureDir, normalizeValue, DEFAULT_KEYWORDS };
