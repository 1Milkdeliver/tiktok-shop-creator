// lib/scraper.js - keyword-pool list scrape + optional detail scrape, with resume state
'use strict';
...
// Extract useful detail fields (bio, email, MCN, etc.) from a flattened profile
...
async function scrapeList(page, keywords, { onProgress, resumeState, isStopped }) {
  ...
  for (const kw of keywords) {
    ...
    while (st.hasMore) {
      if (isStopped && isStopped()) { onProgress && onProgress('已停止'); break; }
      ...
      for (const c of list) {
        ...
        if (id && !seen.has(id)) { seen.add(id); creators.push(slimCreator(c)); added++; }
      }
...
async function scrapeDetails(page, creators, { onProgress, resumeFile, isStopped }) {
...
module.exports = { scrapeList, scrapeDetails, slimCreator, extractDetailFields, FIND_API, PROFILE_API };
