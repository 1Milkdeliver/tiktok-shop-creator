...
class MultiRunner {
  ...
  async start(config) {
    ...
    //   keywords <  sessions : every session scrapes the FULL list (same
    //                          keyword in parallel) — cross-session dedupe
    //                          via sharedSeen prevents duplicate collection
    ...
    if (keywords.length >= sessions.length) {
      ...
      this.log(`关键词 ${keywords.length} 个 < 会话 ${sessions.length} 个 → 同关键词并发（靠共享去重）`);
    }
    ...
    try {
      ...
      const startupPromises = sessions.map((_, i) => (async () => {
        await new Promise(r => setTimeout(r, i * (8000 + Math.random() * 7000)));
  ...
  async runSession(s, keywords, detail, outPath, testMode, dedupe, creatorInput, seenFile, scrapedIds, sharedSeen, sharedDetailSeen, resumeEnabled, mode, sessionsIsolated) {
    ...
    try {
      ...
      if (detail && s.creators.length && !this.stopped) {
  ...
  async openSession(mode, cookieFile, index, isolated) {
    if (mode === 'headless') {
      ...
      // account has separate window/cookies/fingerprint
