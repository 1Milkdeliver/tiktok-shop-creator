// lib/multirunner.js — run multiple cookie sessions concurrently with staggered starts (anti-rate-limit)
'use strict';
...
class MultiRunner {
  ...
  async start(config) {
    ...
    const { cookieFiles, mode, keywords, format, outPath, detail, fields } = config;
    ...
    try {
      this.log(`══════ 多账号并发抓取 ══════`);
      this.log(`Cookie 数: ${sessions.length} | 关键词: ${keywords.length} | 模式: ${mode} | 详情: ${detail ? '是' : '否'}`);
      this.log(`⚠️ 并发策略：各会话错峰启动（间隔 30-60s），降低触发风控概率`);
      ...
      for (let i = 0; i < sessions.length; i++) {
        ...
        try {
          ...
          this.runSession(s, keywords, detail, outPath).catch(e => this.log(`会话 #${i + 1} 错误: ${e.message}`));
        } catch (e) {
  ...
  async runSession(s, keywords, detail, outPath) {
