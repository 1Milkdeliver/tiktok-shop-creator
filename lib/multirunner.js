...
class MultiRunner {
  ...
  async runSession(s, keywords, detail, outPath, testMode) {
    ...
    try {
      ...
      // ── detail phase: batch after list (even if the user stopped the list phase) ──
      if (detail && s.creators.length) {
        const target = s.creators.filter(c => !detailDoneIds.has(c == null ? '' : String(c.creator_oecuid)));
        this.log(`会话 #${s.index + 1}: 正在为 ${target.length} 位达人补抓详情（简介/邮箱/MCN/佣金率）...`);
        for (const c of target) {
          if (this.detailStopped) break; // 2nd stop click aborts detail phase
