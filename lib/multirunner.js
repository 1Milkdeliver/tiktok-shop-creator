...
const { scrapeList, scrapeDetails, extractDetailFields, flattenProfile } = require('./scraper');
...
class MultiRunner {
  ...
  async runSession(s, keywords, detail, outPath, testMode) {
    ...
    try {
      ...
      // ── 即时详情抓取：列表每页新达人人即时入队，后台 worker 并发消费 ──
      const detailQueue = [];
      ...
      const fetchOneDetail = async (c) => {
        ...
        // type3/5 add niche extras; keep it to 2 calls for speed and risk control.
        let mergedProfile = {};
        let anyOk = false;
        ...
        for (const ptypes of [[1], [2]]) {
          ...
          for (let attempt = 0; attempt < 3 && !ok; attempt++) {
            ...
            // merge (later types win on overlap)
      ...
      // one worker: pull from queue until stopped (2nd click) or queue empty + list done
      const makeWorker = () => async () => {
        while (!this.detailStopped) {
          // paused → wait (but stop click un-pauses, handled by stop())
          while (this.paused && !this.detailStopped && this.running) {
            await this.sleep(700);
          }
          const c = detailQueue.shift();
          if (!c) {
            // list finished & queue drained → worker done
            if (this._listDone) break;
            await this.sleep(1200);
            continue;
          }
          try { await fetchOneDetail(c); } catch (e) { }
          await this.sleep(3000 + Math.random() * 7000); // pace detail requests
        }
      };
      const worker1 = makeWorker();
      const worker2 = makeWorker();
      const workers = Promise.all([worker1(), worker2()]);
      ...
      const res = await scrapeList(s.page, keywords, {
        ...
        sellerId: s.sellerId,
        onNewCreators: (newOnes) => { if (detail) detailQueue.push(...newOnes); },
        onProgress: (m) => {
      ...
      // ── drain the detail queue (even if the user stopped the list phase) ──
      this._listDone = true;
      if (detail && s.creators.length) {
        // wait for workers to consume the queue (they finish when queue is empty)
        const drainStart = Date.now();
        while (detailQueue.length > 0 && !this.detailStopped && Date.now() - drainStart < 120000) {
          await this.sleep(1500);
        }
        // give workers a moment to wrap up remaining in-flight requests
        await Promise.race([workers, this.sleep(15000)]);
      } else {
        this.detailStopped = true; // no details needed → stop workers
        await Promise.race([workers, this.sleep(2000)]);
      }
