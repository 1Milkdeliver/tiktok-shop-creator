// lib/browser.js — browser session management: connect to debug Chrome (9222) or launch real-window Chrome
'use strict';
...
const LANDING_URL = 'https://affiliate.tiktokshopglobalselling.com/affiliate/creator?source_from=seller_affiliate_landing&shop_region=US&shop_id=7494397026366031127&route_migration=1';
...
// Window is positioned off-screen (-32000,-32000) so it stays real-fingerprint but doesn't disturb the user
async function launchRealWindow(cookieFile) {
  ...
  const browser = await puppeteer.launch({
    ...
    args: [
      ...
      '--window-position=-32000,-32000',   // off-screen: real browser, invisible to user
...
// Launch headless Chrome (may be rejected by TikTok fingerprinting)
...
function makeXhrFinder(page, apiPath) {
  return async (body) => {
    return await page.evaluate(async (apiPath, body) => {
      const qs = 'user_language=zh-CN&aid=6556&app_name=i18n_ecom_alliance&device_id=0&device_platform=web&oec_seller_id=7494397026366031127&shop_region=US';
